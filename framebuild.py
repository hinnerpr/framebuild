...
from utils import *
from copy import *
from intersection import Lineseg
...
def flex_rigidity(radius, wall, stiffness=200e9):
	"""stiffness in GPa, radius and wall in mm"""
	r2 = radius
	r1 = r2 - wall
...
class Tube:
	...
	def mass(self):
		"""Return the mass in kg not worrying about butting"""
		return mass(self.get_radius(BOTTOM),
			self.get_wall(BOTTOM), self.length())

	def calc_flex_rigidity(self):
		return flex_rigidity(self.get_radius(BOTTOM), self.centre_wall)
...
======""")
		...
		for attr, caption, print_ref in (
			...
			if print_ref:
				ref_points = mitre.reference_points()
				print(caption, "Mitres at ({}) from tube top".format(
						", ".join(["{:.2f}".format(x)
							for x in ref_points])))

				length = mitre.parent.length()
				bottom_points = [length - x for x in ref_points]

				print(caption, "Mitres at ({}) from tube bottom".format(
						", ".join(["{:.2f}".format(x)
	...
	def _calc_ht_projection(self):
		ht = Lineseg(
				self.head_tube.top[0:2],
				self.head_tube.bottom[0:2])

		baseline = Lineseg(
				array([0.0, 0.0]),
				array([self.seat_tube.top[1], -self.seat_tube.top[0]]))

		ix = ht.intersect(baseline)
		self.ht_proj = ix

	def ht_projection(self):
		return (norm(self.ht_proj),
			norm(self.ht_proj - self.head_tube.tube_bottom[:2]))

	def standover(self, distance):
		tt = self.top_tube
		p = tt.bottom + tt.vecn * distance
		p += turn_left(tt.vecn) * tt.radius
		bb_height = self.calc_clearance()[0]
		return p[1] + bb_height
...
-------------""")
		...
		# Useful for setting up my jig
		htp = self.ht_projection()
		print("HT projection to baseline perpendicular "
			"to ST through BB: {:.2f}".format(htp[0]))
		print("Distance from that point to bottom of HT: {:.2f}".format(htp[1]))

		# If this comes out equal to their inseam (or ideally a bit lower) then
		# it should be OK.
		print("Standover 8\" in front of top of ST: {:.2f}".format(
			self.standover(8*25.4)))
	...
	def render_scale_diagram(self, render=None, px_per_mm=2):
		...
		# No point actually drawing this but it was useful for debugging
# 		r.draw_point(self.ht_proj, "red")

		# Same here
# 		tt = self.top_tube
# 		p = tt.bottom + tt.vecn * (25.4 * 8)
# 		p += turn_left(tt.vecn) * tt.radius
# 		r.draw_point(p, "red")
