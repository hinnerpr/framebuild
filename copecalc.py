...
class Curve:
	def __init__(self, cut, par, wall,
			angle, rotate=False, offset=0, taper=0.0, twist=0.0, flat=False):
	...
	def make_points(self):
		...
		for theta in angles():
			...
			if a > 1 or a < -1:
				...
				# The same as the calculation below but cos(phi) is just always
				# 1, because we're always at a constant height regardless of
				# how far we are around the parent tube.
				r = 1.0 + b * taper_r
				y = 1 - r - b * slope
			else:
	...
	def render_png(self, outfile, caption=None, pxi=100):
		...
		legend = "Cut: {}\nWall: {}mm\nAngle: {:.2f}°".format(
				self.ellipse.text(), self.wall, self.angle)

		if self.flat:
			legend = "{}\nFlat".format(legend)
		else:
			legend = "{}\nParent: {}mm".format(legend, self.par)
		if self.offset:
...
""")
	...
	ap.add_argument("--flat", action="store_true", default=False,
				 help="Flat cut, i.e. as if parent diameter were infinity")
	...
	curve = Curve(cut, args.parent, args.wall,
			...
			args.taper, args.twist, args.flat)
